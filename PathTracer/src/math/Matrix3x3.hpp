...
namespace Guarneri
{
	...
	Matrix3x3::Matrix3x3(const float& _m00, const float& _m01, const float& _m02,
						 const float& _m10, const float& _m11, const float& _m12,
						 const float& _m20, const float& _m21, const float& _m22)
	{
		m00 = _m00;
	...
	Matrix3x3 Matrix3x3::rotation(const Vector3& axis, const float& theta)
	{
	...
	const float& Matrix3x3::at(const int& row, const int& column) const
	{
		return this->operator[](rc2index(row, column));
	}

	float& Matrix3x3::at(const int& row, const int& column)
	{
		return this->operator[](rc2index(row, column));
	}
	...
	int Matrix3x3::rc2index(const int& row, const int& column) const
	{
		return row * 3 + column;
	}
